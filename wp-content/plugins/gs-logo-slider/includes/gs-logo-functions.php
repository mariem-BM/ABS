...
function gs_logo_member_description( $max_length = 100, $echo = false, $is_excerpt = true ) {
    ...
    if ( $max_length > 0 && strlen($description) > $max_length ) {
        $gs_logo_more = gs_logo_getoption('gs_logo_more', 'More');
        $description = substr( $description, 0, $max_length ) . sprintf( '...<a href="%s">%s</a>', get_the_permalink(), $gs_logo_more );
    }

    return gs_logo_echo_return( $description, $echo );

}

function gs_logo_member_thumbnail( $echo = false, $size = 'full' ) {

    $member_id = get_the_ID();

    if ( has_post_thumbnail() ) {

        $size = apply_filters( 'gs_logo_member_thumbnail_size', $size, $member_id );
        if ( empty($size) ) $size = 'full';

        $classes = (array) apply_filters( 'gs_logo_thumbnail_classes', ['gs_logo_member--image'] );
        ...
        $thumbnail = sprintf( '<img src="%s" alt="%s" itemprop="image"/>', GSL_PLUGIN_URI . '/assets/img/no_img.png', get_the_title() );

    }

    $thumbnail = apply_filters( 'gs_logo_member_thumbnail_html', $thumbnail, $member_id );

    return gs_logo_echo_return( $thumbnail, $echo );
...
function gs_logo_member_location( $separator = ', ' ) {
    return gs_logo_get_terms_names( 'logo_location', $separator );
}
